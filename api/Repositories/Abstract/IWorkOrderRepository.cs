...
namespace SmartFactoryCMMS.Api.Repositories.Abstract
{
    ...
    {
        Task<PagedResult<WorkOrderListDto>> GetWorkOrdersAsync(int page, int pageSize, string? status = null, string? type = null, CancellationToken ct = default);
        Task<WorkOrderDetailDto?> GetWorkOrderDetailsAsync(Guid id, CancellationToken ct = default);
